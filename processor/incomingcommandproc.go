...
func (cq *InCommandQueue) backend() {
	for  cmd := range cq.commands {
		if (cmd != nil){
			switch cmd.OpCode {
				case "put" : cq.put(cmd.Obj)
				case "delete" : cq.delete(cmd.Obj)
				case "touch" : cq.touch(cmd.Obj)
				case "updatevalue" : cq.updatevalue(cmd.Obj)
				case "updatekey" : cq.updatekey(cmd.Obj)
				case "updatekeyvalue" : cq.updatekeyvalue(cmd.Obj)
